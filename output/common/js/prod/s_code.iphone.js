...
/* change beloow line "rakuten-tw-prod" for Dev */
accountSetting.defaultRSID = "rakuten-tw-prod";
accountSetting.serviceName = "taiwan";
accountSetting.cookieDomainPeriods="3";
accountSetting.currencyCode = "TWD";
...
accountSetting._internalSite[1] = "rakuten.com.tw";
accountSetting._internalSite[2] = "rakuten.tw";
...
s._localVersion = "H.22.1-1.20120307";
...
function s_doPlugins(s) {
				...
				default:
					if (s.getRefGroupsite()=="y"&&s.objCMnew.referringDomain) {
						...
						for (var _idxChn in s.__senew) {
							if (_idxChn != "Paid" && _idxChn != "Group" && _idxChn != "AD:External") { // Paid,Group浠ュ?
	...
	// internal link n-id
	s.eVar29 = s.getQueryParam("n-id", _url.href);
...
// Channel Manager Parameter config
	...
	s.eVar19=s.getQueryParam('l-id');
	s.eVar20=s.getQueryParam('s-id');
	s.eVar21=s.getQueryParam('e-id');
	s.eVar23=s.getQueryParam('f-id');
	
	/* Time Parting */
	//s.prop45=s.getTimeParting('h','8'); // Set hour
	//s.prop46=s.getTimeParting('d','8'); // Set day
	//s.prop47=s.getTimeParting('w','8'); // Set weekday
	//s.eVar45=s.prop45;
	//s.eVar46=s.prop46;
	//s.eVar47=s.prop47;
...
//120713
	if(s.server.split('.')[1]=="shop"){
		//Shop
		s.prop48="Shop";
	}else if(s.server.split('.')[0]=="www"){
		if(location.pathname.split('/')[1]=="event"){
			if(location.pathname.split('/')[2]=="point"){
				//Point
				s.prop48="Point";
			}else{
				//Event
			...
			//Category
			s.prop48="Category";
		}else if(location.pathname.split('/')[1]=="beginner"){
			//Point
			s.prop48="Point";
		}else if(location.pathname.split('/')[1]=="my"){
			//MyRakuten
			s.prop48="MyRakuten";
		}else if(location.pathname.split('/')[1]=="footprints"){
			//Browsing history
			s.prop48="Browsing history";
		}else if(location.pathname.split('/')[1]=="ranking"){
			//Ranking
			s.prop48="Ranking";
	...
	}else if(s.server.split('.')[0]=="search"){
		if(location.pathname.split('/')[1]!=""){
			//Category
			s.prop48="Category";
		}else{
			if(s.getQueryParam("d")!=""){
			//Category
				s.prop48="Category";
			}else{
				//Search
	...
	}else if(s.server.split('.')[0]=="my"){
		if(location.pathname.split('/')[1]=="point"){
			//Point
			s.prop48="Point";
	...
	}else if(s.server.split('.')[0]=="review"){
		//Review
		s.prop48="Review";
	}else if(s.server.split('.')[0]=="step"){
		if(location.pathname.split('/')[1]=="Review"){
		//Reveiw
		s.prop48="Review";
		}
	}else if(s.server.split('.')[0]=="cart"||s.server.split('.')[0]=="checkout"){
		//Order
		if(location.pathname.split('/')[3]=="complete"){
			s.prop48="Order";
		}else{
			//Cart
			s.prop48="Cart";
